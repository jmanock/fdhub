...
export default function FloridaRightNow() {
  ...
  const cards = [
    ...
    ["Beach conditions", unavailable ? "Unavailable" : clearwater?.weather?.summary?.value || "Unavailable", unavailable ? "Official source update pending" : clearwater?.waterTemperature?.value != null ? `Water ${Math.round(clearwater.waterTemperature.value)}°F` : "Water reading unavailable"],
    ...
    ["Cruise ports", unavailable ? "Unavailable" : `${liveData.cruisePorts?.length || 0} ports with tide context`, "Verify operations with port"],
    ["Tropical outlook", unavailable ? "Unavailable" : activeSystems ? `${activeSystems} active system record${activeSystems === 1 ? "" : "s"}` : "No active systems in feed", "Official NHC source"],
  ];

  return (
    <section id="florida-right-now-preview" className="florida-now-preview section-pad" aria-labelledby="hub-live-heading">
      ...
      <div className="florida-now-preview__footer"><span>{unavailable ? "Current official alert data is temporarily unavailable" : alerts ? `${alerts} official alert record${alerts === 1 ? "" : "s"} found` : "No active official alert records found"}</span><span>{unavailable ? "Last successful package" : "Conditions updated"} <time dateTime={liveData.generatedAt}>{formatUpdate(liveData.generatedAt)}</time> · Sources: NWS, NOAA, NHC</span></div>
    </section>
  );
}

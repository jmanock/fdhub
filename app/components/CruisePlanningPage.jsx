  ...
  return (
    <>
      ...
      <main>
        ...
        {guide.durationComparison ? <section className="comparison-section section-pad" aria-labelledby={`${guide.slug}-duration-title`}><div className="section-heading"><p className="eyebrow">Weekend cruise lengths</p><h2 id={`${guide.slug}-duration-title`}>2-Night vs 3-Night vs 4-Night Cruises</h2></div><div className="comparison-table-wrap"><table className="comparison-table"><thead><tr><th>Length</th><th>Trip shape</th><th>Planning tradeoff</th><th>Best for</th></tr></thead><tbody>{guide.durationComparison.map((row) => <tr key={row[0]}>{row.map((cell, index) => index === 0 ? <th scope="row" key={cell}>{cell}</th> : <td key={cell}>{cell}</td>)}</tr>)}</tbody></table></div></section> : null}

        {guide.planningSteps ? <section className="cruise-port-section section-pad" aria-labelledby={`${guide.slug}-steps-title`}><div className="section-heading"><p className="eyebrow">A practical weekend plan</p><h2 id={`${guide.slug}-steps-title`}>Build The Trip In The Right Order</h2></div><div className="cruise-port-map" aria-label="Weekend cruise planning steps">{guide.planningSteps.map(([title, copy], index) => <article key={title}><span>{index + 1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></section> : null}
        ...
        {guide.clusterLinks ? <section className="related-pages section-pad" aria-labelledby={`${guide.slug}-cluster-title`}><div className="section-heading"><p className="eyebrow">Weekend cruise planning path</p><h2 id={`${guide.slug}-cluster-title`}>Compare Sailings, Ports & Complete Packages</h2><p>Use these focused guides to move from a short-cruise idea to a complete Florida departure plan.</p></div><div className="popular-link-grid">{guide.clusterLinks.map(([label, href]) => <Link href={href} key={href}>{label}</Link>)}</div></section> : null}

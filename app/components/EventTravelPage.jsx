...
export default function EventTravelPage({ page }) {
  ...
  const faqs = page.worldCup2026 ? [
    ["Where should I stay for a Florida World Cup trip?", "Choose the hotel area around the verified event and transportation plan first. Miami, Miami Beach, Fort Lauderdale, Orlando, and Port Canaveral support very different trip styles."],
    ["How many days should I visit Florida?", "A focused Miami trip can fit a long weekend, while adding Orlando, Port Canaveral, the Keys, or another region usually deserves several additional days."],
    ["What attractions are nearby?", `Useful options around this guide include ${page.fanExperiences?.join(", ") || "beaches, neighborhoods, museums, theme parks, and family activities"}. Keep fixed event timing and transportation in the plan.`],
    ["What airport should I use?", "MIA and FLL are the strongest airports for the Miami host-city anchor. MCO is useful for a deliberate Orlando vacation extension, not a quick match-day transfer."],
    ["What should families do between matches?", "Families can compare beaches, pools, museums, aquariums, theme parks, Kennedy Space Center, and flexible neighborhood days with enough rest before fixed events."]
  ] : [
  ...
  return (
    <>
      ...
      <main>
        ...
        {page.subpages.length || childTrendPages.length ? <section className="travel-guides section-pad" aria-labelledby={`${page.slug}-guides-title`}><div className="section-heading"><p className="eyebrow">Focused event travel guides</p><h2 id={`${page.slug}-guides-title`}>Plan Every Part Of {page.title}</h2></div><div className="guide-card-grid">{page.subpages.map(([slug, title, description]) => { const worldCupPage = page.worldCup2026 ? getWorldCup2026Page([slug]) : null; return <Link className="guide-card" href={worldCupPage ? worldCup2026Path(worldCupPage) : `/events/${page.slug}/${slug}`} key={slug}><h3>{title}</h3><p>{description}</p></Link>; })}{childTrendPages.map((item) => <Link className="guide-card" href={eventTravelPath(item)} key={item.slug}><h3>{item.title}</h3><p>{item.description}</p></Link>)}</div></section> : null}
        ...
        {page.worldCup2026 ? <WorldCupUpdates /> : null}
